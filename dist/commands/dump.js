...
import { detectAgents, listSupportedAgents, renderAgentTarget, resolveAgent, } from "../agents.js";
function addPendingWrite(pendingWrites, filePath, contents, label) {
    ...
    pendingWrites.set(absPath, {
        filePath,
        contents,
        labels: label ? new Set([label]) : new Set(),
    });
}
function printSupportedAgents() {
    ...
    for (const agent of listSupportedAgents()) {
        ...
        console.log(`  ${chalk.cyan(agent.id.padEnd(8))} ${agent.examplePath}${chalk.dim(aliases)}`);
    }
    console.log();
    console.log(chalk.dim("Use --agent <name> to target one, --all for detected targets, or --target <file> for a custom path."));
}
export async function dumpCommand(options) {
    ...
    if (options.all) {
        console.log(chalk.yellow("No supported agent files detected. Use --agent <name> or --target <file> to create one."));
        return;
    }
    if (options.copy) {
