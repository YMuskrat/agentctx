...
export function forgetCommand(id: string | undefined, options: ForgetOptions): void {
  if (options.undo) {
    undoLastRemove();
    return;
  }
  ...
  const idx = store.entries.findIndex((e) =>
    e.id.slice(0, 8).toLowerCase() === partial
  );
  ...
  const [removed] = store.entries.splice(idx, 1) as [ContextEntry];
  writeContext(store);
  ...
  console.log(
    chalk.green(
      `Removed ${removed.type}: "${removed.content}" (id: ${removed.id.slice(0, 8)})`
    )
  );
}

function undoLastRemove(): void {
  const history = readHistory();

  // Find last remove event
  let lastRemove: (typeof history)[number] | undefined;
  ...
  console.log(
    chalk.green(
      `Restored ${entry.type}: "${entry.content}" (id: ${entry.id.slice(0, 8)})`
    )
  );
}
