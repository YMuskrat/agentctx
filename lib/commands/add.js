...
async function add(args) {
  ...
  console.log('\n' + c.green('✓') + ` Added ${c.bold(bannerName)} ${c.gray('[' + entry.id + ']')}${pinned ? c.yellow('  PINNED') : ''}`);
  console.log(`  ${preview}`);
  console.log(c.gray(`  Next: agenctx view ${entry.id}  ·  ${pinned ? 'agenctx unpin' : 'agenctx pin'} ${entry.id}\n`));
}
