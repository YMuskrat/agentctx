...
function transition(id, newStatus) {
  ...
  const msgs = {
    pinned: `📌 Pinned ${c.gray('[' + id + ']')} — always shown to agents`,
    active: `✓ ${oldStatus === 'pinned' ? 'Unpinned' : 'Restored'} ${c.gray('[' + id + ']')} — back in lifecycle`,
    archived: `📦 Archived ${c.gray('[' + id + ']')} — hidden from agents`,
  };
  console.log(c.green(msgs[newStatus] || `✓ [${id}] → ${newStatus}`));
}
