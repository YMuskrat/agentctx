...
function viewOverview(root, state, config, decayed, agent) {
  ...
  console.log(c.gray('  Type             Entries   Pinned   Purpose'));
  console.log(c.gray('  ───────────────  ───────   ──────   ─────────────────────────────────────────'));
  for (const bannerName of banners) {
    ...
    const purpose = truncate(store.bannerDescription(config, bannerName), 42);
    console.log(`  ${prefix}  ${count}   ${pinnedCount}   ${purpose}`);
    console.log(c.gray(`                                        agenctx view ${bannerName}`));
  }
