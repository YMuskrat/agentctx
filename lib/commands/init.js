...
const { DEFAULTS: DEFAULT_DECAY } = require('../decay');
...
function printPoster() {
  console.log();
  for (const line of LLAMA_POSTER) console.log(c.cyan(line));
  console.log();
  console.log(`  ${c.bold('agenctx')}  ${c.gray('durable context for coding agents')}`);
  console.log();
}
...
async function init(args) {
  ...
  store.saveProposals(cwd, { version: 1, proposals: [] });
