...
function printResults(results, query) {
  ...
  console.log(`\n  ${c.bold(results.length + ' result' + (results.length !== 1 ? 's' : ''))}\n`);
  ...
  for (const [bannerName, entries] of Object.entries(byBanner)) {
    ...
    for (const e of entries) {
      ...
      const pinIcon = e.status === 'pinned' ? ' 📌' : '';
      const reads = e.read_count > 0 ? c.gray(` read ${e.read_count}×`) : '';
      console.log(`     ${c.gray(e.id)}  ${truncate(firstLine, 62)}${pinIcon}${reads}`);
    }
    console.log();
  }

  console.log(c.gray('  Open an entry: agenctx view <id>\n'));
}
...
async function search(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i].startsWith('--since=')) { since = parseSince(args[i].slice(8)); }
